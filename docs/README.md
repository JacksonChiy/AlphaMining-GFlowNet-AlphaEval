...
- [项目架构](architecture.md)：模块职责、依赖方向和运行边界。
- [端到端工作流](workflow.md)：输入、处理、输出及文件流向。
- [实验管理](experiments.md)：实验命名、配置冻结与复现要求。
- [常见故障](troubleshooting.md)：Colab、DolphinDB、MemMap、PPU、LightGBM 和 RQAlphaPlus 排错。
- [方法说明](methodology.md)：Reward、AlphaEval、标签和防泄漏原则。
...
`reports/` 保存阶段性分析、路线图和历史交接材料。它们用于追溯结论，不替代当前配置与运行手册。
