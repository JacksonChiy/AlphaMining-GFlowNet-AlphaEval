...
实现使用向量化分组排名和 NumPy 聚合计算日度 IC，避免逐交易日 Python 回调；RRE 一次性计算相邻日期矩阵。DPP 对交易日做确定性均匀抽样，并复用截面排名相关矩阵，默认最多使用 `dpp_max_rows` 行。该抽样只影响多样性近似，不影响 IC、ICIR、Sharpe、滚动稳定性和扰动鲁棒性的全量计算。
